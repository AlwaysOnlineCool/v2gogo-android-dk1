 ...
 * <p/>
 * The way to use this class is to a) instantiate it programatically or b) create it in your xml
 * layout.
 * <p/>
 * a) The MediaController will create a default set of controls and put them in a window floating
 * above your application. Specifically, the controls will float above the view specified with
 * setAnchorView(). By default, the window will disappear if left idle for three seconds and
 * reappear when the user touches the anchor view. To customize the MediaController's style, layout
 * and controls you should extend MediaController and override the {#link
 * {@link #makeControllerView()} method.
 * <p/>
 * b) The MediaController is a FrameLayout, you can put it in your layout xml and get it through
 * {@link #findViewById(int)}.
 * <p/>
 * NOTES: In each way, if you want customize the MediaController, the SeekBar's id must be
 * mediacontroller_progress, the Play/Pause's must be mediacontroller_pause, current time's must be
 * mediacontroller_time_current, total time's must be mediacontroller_time_total, file name's must
 * be mediacontroller_file_name. And your resources must have a pause_button drawable and a
 * play_button drawable.
 * <p/>
...
public class MediaController extends FrameLayout
{
	...
	private void initControllerView(View v)
	{
		...
		mProgress = (CustomSeekBar) v.findViewById(getResources().getIdentifier("mediacontroller_seekbar", "id", mContext.getPackageName()));
		if (mProgress != null)
		{
	...
	public void setSeekAble(boolean isSeekAble)
	{
		if (null != mProgress)
		{
			mProgress.setSeekAble(isSeekAble);
		}
	}
	 ...
	 * <p/>
	 * <p>
	 * If the controller is showing, calling this method will take effect only the next time the
	 * controller is shown.
	 * </p>
	 * 
